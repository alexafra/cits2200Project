...
public class Sorter implements Sort
{
    ...
    {
        int n = q - p + 1; 	//length of left array
        int m = r - q;		//length of right array

        long[] an = new long[n]; 	//left array
        long[] am = new long[m];	//right array
        ...
        //k is the actual array index
        for (int k = p; k <= r; k++) {
            if ( i == n ) { //if left array copied only copy from right
                a[k] = am[j++];
            } else if ( j == m || an[i] < am[j] ) { //if both arent copied determine which is smaller
                a[k] = an[i++];
            }
            else { //if right array is copied, only copy left
                a[k] = am[j++];
            }
            count++;
        }
